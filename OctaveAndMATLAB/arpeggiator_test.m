function [scale, note_name] = ...
    arpeggiator_test(chord_number, transposed, direction, bars)
%ARPEGGIATOR_TEST Creates chords/arpeggiator patterns
%
%   ARPEGGIATOR_TEST([note_number],[transposed],[direction],[bars])
%
%   Example:
%     arpeggiator_test(8, 4, 1, 2)
%
%   Returns:
%     scale: Note values
%     note_name: Note names
%
%   See also: ARPEGGIATOR
...
%         Beats pr. measure (3/4, 4/4, etc.)
    ...
    FIGURE_NAME = 'Arpeggiator/Chord Generator';
    SEMI_TONES = 12;
    ...
    for counter = 1:len
        if bitget(chord_notes(chord_number), mod((p - 1), ...
            ...
            scale(counter) = mod((counter - 1 + transposed), ...
                SEMI_TONES) + 1;
            octave_no = floor((SEMI_TONES + counter + ...
                transposed - 1) / SEMI_TONES);             
            note_name{counter} = [noteletters{mod((counter - 1 + ...
