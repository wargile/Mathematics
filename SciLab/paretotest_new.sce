...
//                         [get_maximum_ten_elements true|false])
    ...
    set(H2, 'Marker', 'none'); // No plot marker
    ...
    if left_y_axis_tight == false
        ...
        y1_ticks = linspace(0, sum(data), MAX_PERCENT_TICKS);
    else
        set(AX(1), 'YLim', [0, max(data)]);
        y1_ticks = linspace(0, max(data), MAX_PERCENT_TICKS);
        //axis tight
    end

    //for i = 1:length(y1_ticks) - 1 % Round of all but last
    //    y1_ticks(i) = floor(y1_ticks(i));
    //end
